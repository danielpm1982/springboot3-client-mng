...
public class Bootstrap {
    ...
    public Bootstrap(ClientRepositoryInterface clientRepositoryInterface) {
    ...
    private void testAddClients(){
        ...
        clientRepositoryInterface.saveClient(c1);
        clientRepositoryInterface.saveClient(c2);
        clientRepositoryInterface.saveClient(c3);
        logger.log(Level.INFO, "Clients Saved !");
    }
    private void testListAllClients(){
        logger.log(Level.INFO, "Listing All Clients:");
        clientRepositoryInterface.findAllClients().forEach(x->logger.log(Level.INFO,x));
    }
    private void testGetSingleClient(){
        logger.log(Level.INFO, "Finding Client Id=1");
        logger.log(Level.INFO,clientRepositoryInterface.findClientById(1L));
    }
    private void testUpdateSingleClient(){
        logger.log(Level.INFO, "Updating Client Id=1 with name \"updated\" at the end:");
        Client c1 = clientRepositoryInterface.findClientById(1L);
        c1.setClientName(c1.getClientName()+"-updated");
        logger.log(Level.INFO,clientRepositoryInterface.updateClient(c1));
    }
    private void testDeleteSingleCLient(){
        logger.log(Level.INFO, "Delete Client Id=3:");
        clientRepositoryInterface.deleteClientById(3L);
        clientRepositoryInterface.findAllClients().forEach(x->logger.log(Level.INFO,x));
    }
